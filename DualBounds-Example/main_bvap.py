...
# Model imports
import labeling, ordering, fixing, separation
...
############################################################
        if 'bvap_bounds' in obj or 'bvap' in ordering:
    ...
    ############################  
        ...
        def build_relax_model(G, DG, population, R, k, bvap_ws, vap_ws):
            ...
            #fn = f"../results_for_{config_filename_wo_extension}/{result['state']}-{result['level']}-{config['obj']}-{config['R']}"
        ...
        run_time_limit_ub = 3600#2400 #1200 #600 # 5 minutes
        run_time_limit_lb = 60 # 30 seconds
        ...
        for cut_num in range(counter, num_grad_cuts + counter):
            ...
            result['gradient_bounds'][cut_num]['ub objective value'] = ....
            if result['gradient_bounds'][cut_num]['ub objective value'] > float(obj_value_ws):
                obj_value_ws = result['gradient_bounds'][cut_num]['ub objective value']
        ...
        for cut_num in result['gradient_bounds'].keys():
            ...
            m.addConstr( gradient >= result['gradient_bounds'][cut_num]['lb'], 
                                      name = f"Grad_cut_lb{cut_num}"  )  
        m_bounds.Params.TimeLimit = 3600
        m.update()
